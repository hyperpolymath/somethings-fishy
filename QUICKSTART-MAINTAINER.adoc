...
// Replace rsr-template-repo, somethings-fishy, {{DEPS}} with actuals
...
| `$PREFIX/share/somethings-fishy/`
| Data files, assets

| `$PREFIX/share/doc/somethings-fishy/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/somethings-fishy/config.toml`

Fallback: `$HOME/.config/somethings-fishy/config.toml`
...
Or via OPSM: `opsm update somethings-fishy`
...
[source,bash]
----
just install --prefix=/opt/somethings-fishy-instance1 --config=/etc/somethings-fishy/instance1.toml
just install --prefix=/opt/somethings-fishy-instance2 --config=/etc/somethings-fishy/instance2.toml
----
