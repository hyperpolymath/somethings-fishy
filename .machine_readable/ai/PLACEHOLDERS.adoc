...
sed -i 's/Something's [Robo] Fishy/my-project/g' $(grep -rl 'Something's [Robo] Fishy' .)
sed -i 's/SOMETHINGS_FISHY/MY_PROJECT/g' $(grep -rl 'SOMETHINGS_FISHY' .)
sed -i 's/somethings_fishy/my_project/g' $(grep -rl 'somethings_fishy' .)
sed -i 's/somethings-fishy/my-project/g' $(grep -rl 'somethings-fishy' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `Something's [Robo] Fishy` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `Agent-damage triage tool — checks whether LLM agents have broken a repo` | One-line description | `A tool for X` | flake.nix |
| `SOMETHINGS_FISHY` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `somethings_fishy` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `somethings-fishy` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
...
| `https://github.com/hyperpolymath/somethings-fishy` | Project website | `https://example.org` | SECURITY.md |
