...
# SOMETHINGS_ROBOFISHY ABI/FFI Documentation
...
│  generated/abi/somethings_robofishy.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libsomethings_robofishy.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── somethings_robofishy.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── somethings_robofishy.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/somethings_robofishy.h
```
...
```c
#include "somethings_robofishy.h"

int main() {
    void* handle = somethings_robofishy_init();
    if (!handle) return 1;

    int result = somethings_robofishy_process(handle, 42);
    if (result != 0) {
        const char* err = somethings_robofishy_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    somethings_robofishy_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lsomethings_robofishy -L./zig-out/lib
```

### From Idris2

```idris
import SOMETHINGS_ROBOFISHY.ABI.Foreign
...
```rust
#[link(name = "somethings_robofishy")]
extern "C" {
    fn somethings_robofishy_init() -> *mut std::ffi::c_void;
    fn somethings_robofishy_free(handle: *mut std::ffi::c_void);
    fn somethings_robofishy_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = somethings_robofishy_init();
        assert!(!handle.is_null());

        let result = somethings_robofishy_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libsomethings_robofishy = "libsomethings_robofishy"

function init()
    handle = ccall((:somethings_robofishy_init, libsomethings_robofishy), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:somethings_robofishy_process, libsomethings_robofishy), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:somethings_robofishy_free, libsomethings_robofishy), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/somethings_robofishy.h
   ```
