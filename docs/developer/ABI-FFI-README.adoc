...
# SOMETHINGS_FISHY ABI/FFI Documentation
...
│  generated/abi/somethings_fishy.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libsomethings_fishy.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── somethings_fishy.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── somethings_fishy.h       # Generated from Idris2 ABI
│
...
idris2 --cg c-header Types.idr -o ../../generated/abi/somethings_fishy.h
```
...
```c
#include "somethings_fishy.h"

int main() {
    void* handle = somethings_fishy_init();
    if (!handle) return 1;

    int result = somethings_fishy_process(handle, 42);
    if (result != 0) {
        const char* err = somethings_fishy_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    somethings_fishy_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lsomethings_fishy -L./zig-out/lib
```

### From Idris2

```idris
import SOMETHINGS_FISHY.ABI.Foreign
...
```rust
#[link(name = "somethings_fishy")]
extern "C" {
    fn somethings_fishy_init() -> *mut std::ffi::c_void;
    fn somethings_fishy_free(handle: *mut std::ffi::c_void);
    fn somethings_fishy_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = somethings_fishy_init();
        assert!(!handle.is_null());

        let result = somethings_fishy_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libsomethings_fishy = "libsomethings_fishy"

function init()
    handle = ccall((:somethings_fishy_init, libsomethings_fishy), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:somethings_fishy_process, libsomethings_fishy), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:somethings_fishy_free, libsomethings_fishy), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/somethings_fishy.h
   ```
